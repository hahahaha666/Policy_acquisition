...
class producer(object):
    ...
    def select_SQL(self):
        ...
        NOT_js=SQLhy_not_js(conn,cursor)
        ...
        conn, cursor = create_conn()
        DIS_info = SQLhy_disposition(conn, cursor)
        close_conn(conn, cursor)
...
def ins_one(item,verdict):
    ...
    try:
        conn, cursor = create_conn()
        sql = " UPDATE  tbl_industry_details  SET release_at=%s,source=%s,content=%s,attachment_id=%s,type=0,title=%s,status=1,img_md5=%s  WHERE url=%s"
        cursor.execute(sql, (c_time, source, content, ",".join(md), title, img_md, url))
        conn.commit()
        print(" 更新详情表成功" + str(title) + str(c_time) + "  " + str(url))
        close_conn(conn, cursor)
...
def fir_img_ins( content):
    ...
    if imgs:
        for i in imgs:
            ...
            try:
                conn, cursor = create_conn()
                sql_i = "insert into tbl_industry_img(md5,url,status,path) values (%s,%s,%s,%s)"
...
def ins_fj(info):
    ...
    try:
        conn, cursor = create_conn()
        sql = "insert ignore into tbl_industry_attachment(name,url_name,url,path,md5,type) values (%s,%s,%s,%s,%s,%s)"
...
def update_sql(url):
    ...
    try:
        conn, cursor = create_conn()
        sql = " UPDATE  tbl_industry_details  SET status=4  WHERE id=%s"
...
def update_fail(url):
    ...
    try:
        conn, cursor = create_conn()
        sql = " UPDATE  tbl_industry_details  SET status=3  WHERE id=%s"
        cursor.execute(sql, url)
        conn.commit()
        close_conn(conn, cursor)
    except Exception as err:
...
def  one(start,Annex_queue,detail_list, verdict, item):
...
if __name__ == '__main__':
    start = producer()
    verdict=[]
    detail_list, item = start.select_SQL()
    Annex_queue = multiprocessing.Queue(maxsize=0)  ###队列 用来放信息的  1代表详情页  2附件  3日志  4更新  5采集失败
    p1 =  multiprocessing.Process(target=one, args=(start,Annex_queue,detail_list, verdict, item,))
