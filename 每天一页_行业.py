...
class WORK():
    ...
    def update_tttsql(self):
        try:
            conn, cursor = create_conn()
            sql = "update tbl_industry_info set eveday_status=0 where id>0 "
            cursor.execute(sql)
            conn.commit()
            close_conn(conn, cursor)
        except:
...
def ins_info(item):
    ...
    if pd:
        ...
        try:
            conn, cursor = create_conn()
            sql = """
                                                  insert into tbl_industry_details(post_data,title,cid,type,status,release_at,url) values (%s,%s,%s,%s,%s,%s,%s)
                                                  """
        ...
        if num == 1:
            try:
                conn, cursor = create_conn()
                sql = " UPDATE  tbl_industry_details  SET post_data=%s, title=%s,type=%s,status=%s,release_at=%s  WHERE url=%s and cid=%s"
...
def  select_cf(url):
    sql_i = "select id from tbl_industry_details where url=%s"
    try:
        conn, cursor = create_conn()
        cursor.execute(sql_i, url)
        all = cursor.fetchall()
        close_conn(conn, cursor)
    except  Exception as err:
        print(err,2)
...
def up_eday(cid):
    ...
    sql = "update tbl_industry_info set eveday_status=1 where id=%s "
    try:
        conn, cursor = create_conn()
